...
function StuckEmotionFigure({ size }) {
  const s = size==="sm" ? 0.85 : 1;
  const W = 140, H = 190 * s;
  // Terry-style profile face: nose bump leads into a clearly protruding lip
  // shape (sticks out past the chin line) before the very short neck. Feeling
  // sits up in the chest, just below the shoulders.
  const backOfHead = `M72,14 C 83,14 91,21 91,31 C 91,40 85,47 79,50 C 79,57 78,64 78,79`;
  const faceProfile = `
    M72,14
    C 65,15 60,19 59,25
    C 58,28 60,29 63,28
    ...
    C 65,68 66,72 66,79
  `;
  const neckLeft  = `M66,79 L 65,82`;
  const neckRight = `M76,79 L 77,82`;
  const torso = `M58,82 C 56,95 55,110 56,122 C 57,130 60,135 64,138 C 70,141 80,141 86,138 C 90,135 93,130 94,122 C 95,110 94,95 92,82`;
  const shoulders = `M58,82 C 64,79 86,79 92,82`;
  const leftArm  = `M58,82 C 52,90 47,98 46,108 C 45,116 47,122 50,126`;
  const rightArm = `M90,82 C 96,90 101,98 102,108 C 103,116 101,122 98,126`;
  const leftLeg  = `M64,128 C 62,142 60,156 58,170 C 57,178 58,184 60,188`;
  const rightLeg = `M84,128 C 86,142 88,156 90,170 C 91,178 90,184 88,188`;
  const chestX = 75, chestY = 96;

  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{maxHeight:H+10, display:"block", margin:"0 auto"}}>
      <path d={leftArm}  fill="none" stroke="#FCA88B" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={rightArm} fill="none" stroke="#FCA88B" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={leftLeg}  fill="none" stroke="#FCA88B" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={rightLeg} fill="none" stroke="#FCA88B" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={torso}     fill="none" stroke="#FCA88B" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
      <path d={shoulders} fill="none" stroke="#FCA88B" strokeWidth="2.4" strokeLinecap="round"/>
      <path d={neckLeft}  fill="none" stroke="#FCA88B" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={neckRight} fill="none" stroke="#FCA88B" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={backOfHead} fill="none" stroke="#FCA88B" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
      <path d={faceProfile} fill="none" stroke="#FCA88B" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>

      {/* furrowed brow above the eye — the worried/tense detail */}
      <path d="M58,30 Q61,28 64,30" fill="none" stroke="#2A0F02" strokeWidth="1.4" strokeLinecap="round" opacity="0.7"/>
      <circle cx="66" cy="34" r="1.7" fill="#2A0F02"/>
      {/* sad mouth — drawn right at the protruding lip bump, dipping down */}
      <path d="M64,53 Q71,60 78,52" fill="none" stroke="#2A0F02" strokeWidth="1.8" strokeLinecap="round" opacity="0.85"/>

      {/* the stuck emotion — a tight, knotted tangle sitting up in the chest, not moving */}
      <g transform={`translate(${chestX},${chestY})`}>
        <circle r="12" fill="#7C2D12" opacity="0.55"/>
        <path d="M-7,-4 Q0,-10 7,-4 Q10,0 7,5 Q0,10 -7,5 Q-10,0 -7,-4 Z"
          fill="none" stroke="#FED7AA" strokeWidth="1.6" opacity="0.9"/>
        <path d="M-3,-2 Q0,-4 3,-2 Q4,0 3,2 Q0,4 -3,2 Q-4,0 -3,-2 Z"
          fill="#FED7AA" opacity="0.5"/>
      </g>

      <text x={W/2} y={H-6} textAnchor="middle" fontSize={size==="sm"?9:10} fill="#94a3b8" fontFamily="Inter,sans-serif">
        an emotion, frozen in place
      </text>
    </svg>
  );
}
...
function PointingAtChestFigure({ size }) {
  const s = size==="sm" ? 0.85 : 1;
  const W = 140, H = 190 * s;
  // Same Terry-style profile face as StuckEmotionFigure: prominent protruding
  // lips, very short neck, with a relaxed brow and a clear upward smile, and
  // the right arm bending down to point at the sensation now up in the chest.
  const backOfHead = `M72,14 C 83,14 91,21 91,31 C 91,40 85,47 79,50 C 79,57 78,64 78,79`;
  const faceProfile = `
    M72,14
    C 65,15 60,19 59,25
  ...
  const shoulders = `M58,82 C 64,79 86,79 92,82`;
  const leftArm  = `M58,82 C 52,90 47,98 46,108 C 45,116 47,122 50,126`;
  // Right arm bends down from the shoulder to point at the chest sensation
  const rightArm = `M91,85 C 96,89 98,92 96,96 C 94,99 90,98 88,95`;
  const leftLeg  = `M64,128 C 62,142 60,156 58,170 C 57,178 58,184 60,188`;
  const rightLeg = `M84,128 C 86,142 88,156 90,170 C 91,178 90,184 88,188`;
  const chestX = 75, chestY = 96;
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{maxHeight:H+10, display:"block", margin:"0 auto"}}>
      <path d={leftArm}  fill="none" stroke="#7DD3C0" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={rightArm} fill="none" stroke="#7DD3C0" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={leftLeg}  fill="none" stroke="#7DD3C0" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={rightLeg} fill="none" stroke="#7DD3C0" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={torso}     fill="none" stroke="#7DD3C0" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
      <path d={shoulders} fill="none" stroke="#7DD3C0" strokeWidth="2.4" strokeLinecap="round"/>
      <path d={neckLeft}  fill="none" stroke="#7DD3C0" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={neckRight} fill="none" stroke="#7DD3C0" strokeWidth="2.2" strokeLinecap="round"/>
      <path d={backOfHead} fill="none" stroke="#7DD3C0" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
      <path d={faceProfile} fill="none" stroke="#7DD3C0" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round"/>
      {/* fingertip touching the chest sensation */}
      <circle cx="88" cy="95" r="2.2" fill="#7DD3C0"/>

      {/* relaxed brow above the eye */}
      <path d="M58,29 Q61,28 64,29" fill="none" stroke="#031A22" strokeWidth="1.3" strokeLinecap="round" opacity="0.55"/>
      <circle cx="66" cy="34" r="1.7" fill="#031A22"/>
      {/* happy mouth — drawn right at the protruding lip bump, curving up */}
      <path d="M64,55 Q71,48 78,55" fill="none" stroke="#031A22" strokeWidth="1.8" strokeLinecap="round" opacity="0.85"/>

      {/* the sensation being noticed — calm, settled, up in the chest */}
      <g transform={`translate(${chestX},${chestY})`}>
        <circle r="11" fill="#2DD4BF" opacity="0.35"/>
        <circle r="5.5" fill="#5EEAD4" opacity="0.7"/>
        <circle r="12" fill="none" stroke="#5EEAD4" strokeWidth="1.2" opacity="0.5">
      ...
      <text x={W/2} y={H-6} textAnchor="middle" fontSize={size==="sm"?9:10} fill="#94a3b8" fontFamily="Inter,sans-serif">
        noticing what's here, with curiosity
      </text>
    </svg>
  );
}
...
function SomaticDiagram({ practice, size }) {
  switch (practice.diagram) {
    case "arousal": return <StuckEmotionFigure size={size}/>;
    case "body": return <PointingAtChestFigure size={size}/>;
